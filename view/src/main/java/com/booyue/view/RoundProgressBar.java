...
public class RoundProgressBar extends View {
	...
	//进度条结束角度
	private static int END_DEGREE = 360;
	...
	protected void onDraw(Canvas canvas) {
		...
		switch (style) {
			case STROKE:{
				...
				if(progress !=0) {
				//根据进度画圆弧
					canvas.drawArc(oval, START_DEGREE, END_DEGREE * progress / max, false, paint);
				}
				break;
			}
			case FILL:{
				...
				if(progress !=0) {
					//根据进度画圆弧
					canvas.drawArc(oval, START_DEGREE, END_DEGREE * progress / max, true, paint);
				}
				break;
			}
		}
